...
- En python, las truplas son inmutables, es decir no se pueden cambiar los valores una vez creadas.
- Si modifico la funcion agregar_producto se actualiza la cantidad en lugar de reemplazarlo.
...
En el diccionario las claves son unicas. Si agrego un producto con na clave existente, el valor anterior se reemplaza, esto asegura que cada producto tenga una sola entrad en el invenatario.

# 3. Tuplas en Acceso: 
En la función valor_total_inventario, se utiliza la línea for detalles in inventario.values():. ¿Cómo aprovecha el código la naturaleza ordenada de la tupla detalles para acceder al precio y al stock dentro del bucle sin usar un índice numérico ([0], [1])? ¿Por qué fue una buena decisión de diseño usar una tupla 
en lugar de una lista para almacenar los detalles fijos del producto?

Las truplas son rapidas y seguras, ideal para datos fijos como precio o cantidad.